...
| **Product dimensions in the export** — a Height (or Dimensions) column carried into the generation prompt as a scale cue | The sheet does not have one today, and adding a column is the customer's change to their own file, not ours (#12). The source photo fixes shape, colour and finish, but it is shot on white with nothing beside it, so **nothing in the pipeline knows how big the product is** — a 4 oz bud vase and a 24-inch floor vase are the same photo and the same prompt, and the model settles the question by inventing a scene. It is a cheap fix when it comes: one optional column, read straight through to `buildEditPrompt` (`src/lib/prompts.ts`) beside colour and material, and ignored when absent. It is deferred because the fix belongs *next to the evidence* — knowing which SKUs it went wrong for tells us whether a number is enough or whether the prompt needs "on a dining table, for scale" | **A candidate rejected for scale** — a product that came back plausibly lit and plainly the wrong size. Ambiguous-size SKUs are known in advance (vases, bowls, serving pieces), so the first rejection on one is the signal. Also fires if anyone adds a size column to the sheet on their own |
