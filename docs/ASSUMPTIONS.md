...
- **What it changed:**
  ...
  - **Tension with #5a:** done is product-level at 2+ approved images, so a themed round for an already-done product moves no progress number. Themed batches therefore need to be tracked as their own run. This is exactly the blind spot #5a flagged, arriving sooner than expected — **watch it.**
    - **[resolved in the build]** Tracked as their own run, as this predicted. A drop's progress, its completion and its stuck list all count **within the drop's own theme**, so a holiday round on an everyday-done product reads as "awaiting a decision" in the holiday drop and its candidate message says `needs 1 more` against the holiday set, not the product's total. What keeps the original tension is **product-level** status (`/shots HG-002`), which still says done at 2 images of any kind — deliberately, because that is the question "does this product have styled photos" actually asks. The blind spot that remains is therefore the narrow one: a product done everyday and untouched for holiday is not *stuck*, and appears in no queue, until a holiday drop exists to count it.
