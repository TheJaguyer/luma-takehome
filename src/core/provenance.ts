...
const TRAINED_AI = "http://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia";

/**
 * Flow 5: a photo a person uploaded and declared AI-made (#16). We do not know how it was made,
 * only that it was — so the marker is the plain "trained algorithmic media" one, and the note says
 * the claim came from the uploader rather than from us.
 */
export function uploadedAiProvenanceXmp(p: { sku: string; approvedAt: Date; imageId: string }) {
  const note = escapeXml(`Uploaded photo of ${p.sku}, declared AI-generated by the person who uploaded it.`);
  return `<?xpacket begin="﻿" id="W5M0MpCehiHzreSzNTczkc9d"?>
<x:xmpmeta xmlns:x="adobe:ns:meta/">
 <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  <rdf:Description rdf:about=""
    ...
    xmlns:dc="http://purl.org/dc/elements/1.1/"
    xmlns:xmp="http://ns.adobe.com/xap/1.0/"
    Iptc4xmpExt:DigitalSourceType="${TRAINED_AI}"
    xmp:CreateDate="${p.approvedAt.toISOString()}">
   <dc:identifier>${escapeXml(p.imageId)}</dc:identifier>
   <dc:description><rdf:Alt><rdf:li xml:lang="x-default">${note}</rdf:li></rdf:Alt></dc:description>
  </rdf:Description>
